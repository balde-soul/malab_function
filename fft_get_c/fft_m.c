    ...
    mclOutputHandlerFcn print_handler)
{
  ...
  if (!mclmcrInitialize())
    return false;
  if (!GetModuleFileName(GetModuleHandle("fft_m"), path_to_dll, _MAX_PATH))
    return false;
    {
...
{
  return fft_mInitializeWithHandlers(mclDefaultErrorHandler, mclDefaultPrintHandler);
}
...
{
  return mclFeval(_mcr_inst, "fft_m", nlhs, plhs, nrhs, prhs);
}

LIB_fft_m_C_API 
bool MW_CALL_CONV mlfFft_m(int nargout, mxArray** B, mxArray** C, mxArray** D, mxArray* A)
{
  return mclMlfFeval(_mcr_inst, "fft_m", nargout, 3, 1, B, C, D, A);
}
